...
def p_elemlist_bin(p):
    ...
    if len(p) == 2:
        if p[1] in lines:
            p[0] = tuple([p[1] + '.' + x for x in lines[p[1]]])
        elif p[1] in elements:
            p[0] = (p[1],)
        else:
...
def expand_line(line, elemdict):
    ...
    for elem in line:
        ...
        fam = name.split('.')
        if fam[-1] not in elemdict:
            raise RuntimeError("can not find element '%s'" % fam[-1])
        tag, origname, elemtype, prpts = elemdict[fam[-1]]
        fuprpts = prpts.copy()
...
if _the_line_:
    fuline = expand_line(lines[_the_line_], elements)
